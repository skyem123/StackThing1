...
abstract class ProgramEntry {
	def this(params: Seq[String]) = this
...
object ProgramEntry {
	...
	val instructions = Array[ProgramEntry](nop, add, sub, so, eq, save, load, call, dup, ret, swp, del, clear, swcs, cfcs, ptcs, dfcs)
	...
	class data(val entry: StackEntry) extends ProgramEntry {
		def this(params: Seq[String]) {
			this(stackEntry(params.head, params.tail.head))
		}
